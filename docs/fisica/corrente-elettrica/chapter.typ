...
#intro[La corrente elettrica rappresenta il cuore della maggior parte delle applicazioni dell'elettricità. Quando accendiamo un apparecchio, sia esso una lampada, un elettrodomestico o una radio, questi funzionano grazie al flusso di corrente elettrica al loro interno.

La storia dell'elettricità e dei suoi utilizzi pratici ha conosciuto una svolta decisiva con il passaggio dall'elettricità statica, che riguarda le cariche in stato di quiete, a quella dinamica, legata al movimento delle cariche, cioè alla corrente elettrica. Tale rivoluzione avvenne intorno al 1800 grazie al contributo di Alessandro Volta, fisico italiano che ideò la pila, il primo generatore di corrente elettrica. Questo strumento era capace di generare un flusso continuo di cariche elettriche attraverso un conduttore per un periodo relativamente lungo.]

== Che cos'è la corrente elettrica

Quando si avvicinano due conduttori con una differenza di potenziale tra loro, si genera un flusso di cariche elettriche negative dal conduttore a potenziale inferiore verso quello a potenziale superiore, oppure un flusso di cariche positive nel senso opposto. Questo movimento di cariche è chiamato corrente elettrica e ha il compito di ristabilire l'equilibrio elettrico tra i conduttori. Il flusso cessa una volta raggiunto lo stesso potenziale tra i due.

Le cariche elettriche in movimento possono avere una diversa natura. Nei liquidi e nei gas, la corrente è costituita da cariche positive e negative (ioni positivi e ioni negativi). Nei conduttori metallici, invece, è determinata esclusivamente dal movimento delle cariche negative, ossia degli elettroni.
...
Il flusso di cariche elettriche all'interno di un conduttore, noto come corrente elettrica, persiste finché alle estremità del conduttore è presente una differenza di potenziale. Una volta raggiunto l'equilibrio elettrico, ossia quando il potenziale elettrico risulta uguale in tutti i punti del conduttore, la corrente cessa di circolare.

Questo fenomeno può essere paragonato, in termini di analogia, a ciò che avviene con il movimento di un fluido: una differenza di pressione tra due zone atmosferiche provoca un flusso d'aria dalla zona a pressione maggiore verso quella a pressione minore. Analogamente, nel trasferimento di calore tra due corpi a temperature diverse, l'energia termica fluisce dal corpo più caldo verso quello più freddo fino a che entrambi raggiungono la stessa temperatura.

Allo stesso modo, la corrente elettrica attraversa un conduttore finché tutti i suoi punti non hanno acquisito il medesimo potenziale. Affinché la corrente continui a fluire anche dopo il raggiungimento dell'equilibrio elettrico, è necessario intervenire con un dispositivo capace di mantenere costante la differenza di potenziale. Tale apparato fornisce al conduttore, o al sistema di conduttori, l'energia indispensabile per sostenere il flusso di corrente elettrica al suo interno. Questo dispositivo prende il nome di generatore di tensione o generatore elettrico e ha il compito di annullare gli effetti del movimento naturale delle cariche attraverso il conduttore, movimento che tende naturalmente a eliminare la differenza di potenziale.

Un'analogia idraulica permette di comprendere facilmente la funzione del generatore. Si pensi a un serbatoio d'acqua posto a una determinata altezza dal suolo e collegato, tramite un tubo, a una vasca situata in basso. L'energia potenziale gravitazionale dell'acqua genera spontaneamente un flusso dai punti a energia potenziale maggiore (il serbatoio) verso quelli a energia potenziale minore (la vasca), provocando il trasferimento completo dell'acqua dal serbatoio alla vasca in breve tempo. Per riportare l'acqua dalla vasca al serbatoio sarebbe necessario utilizzare una pompa idraulica, che esercita un lavoro contro le forze gravitazionali.
...
#boxfl(bottom)[== #wolf La pila elettrica <senza_numero>
...
Un circuito elettrico, in generale, è costituito da un insieme di conduttori collegati tra loro e ai poli di un generatore di tensione. 

Il modello più semplice di circuito elettrico si realizza collegando una pila a un filo metallico. All'interno del filo metallico, la corrente elettrica scorre nel senso convenzionale, ossia dal polo positivo al polo negativo. 

Se interrompiamo il filo in un punto e colleghiamo una lampadina alle due estremità, la corrente che attraversa il filo verrà impiegata per illuminare la lampadina. La pila ha il compito di fornire l'energia necessaria alle cariche per muoversi e generare una corrente elettrica. Queste cariche, avanzando lungo il filo metallico, raggiungono la lampadina dove si verifica la trasformazione dell'energia da elettrica a luminosa e termica. Nel caso di una lampadina a incandescenza, la corrente attraversa il filamento, che si riscalda fino a diventare incandescente, producendo luce e calore.

Dopo aver ceduto parte della loro energia alla lampadina, le cariche tornano al polo negativo della pila per "rifornirsi" di nuova energia, ripetendo il processo ciclicamente. 

Nel circuito, il componente che utilizza l'energia elettrica, come la lampadina nel nostro esempio, viene chiamato resistore o carico. Se i conduttori del circuito sono connessi in modo continuo (cioè senza interruzioni nel percorso delle cariche), si parla di circuito chiuso. Al contrario, se la corrente è interrotta anche in un solo punto, il circuito viene definito aperto, condizione nella quale la corrente smette di circolare.

Gli elementi di un circuito possono essere collegati tra loro in diversi modi; tra questi, i più comuni sono il collegamento in serie e quello in parallelo. Nel collegamento in serie, due o più conduttori sono attraversati consecutivamente dalla stessa corrente. Nel collegamento in parallelo, invece, la corrente si divide in due rami separati per poi ricongiungersi alla fine del percorso.

In un circuito in serie, tutti gli elementi devono funzionare contemporaneamente affinché il circuito operi correttamente. Se uno degli elementi si danneggia, il circuito si apre e la corrente smette di circolare. È per questo motivo che nelle abitazioni i circuiti vengono generalmente progettati con collegamenti in parallelo: questa configurazione consente al circuito di continuare a funzionare anche se una parte viene interrotta.

Nei circuiti collegati in parallelo, una porzione del circuito può essere aperta senza compromettere il funzionamento della restante parte. Questo garantisce maggiore flessibilità e affidabilità nei contesti domestici.
...
#boxfl(bottom)[== #wolf I superconduttori <senza_numero>
...
La forza elettromotrice, indicata comunemente con la sigla f.e.m., rappresenta la differenza di potenziale massima che un generatore elettrico è in grado di fornire.
...
#boxfl(bottom)[== #wolf Il diodo <senza_numero>
...
#boxfl(top)[== #wolf Il transistor <senza_numero>
...
L'energia elettrica è estremamente utile per l'uomo principalmente perché può essere facilmente trasformata in altre forme di energia, in particolare energia termica, ossia calore.

Un esempio di questa trasformazione si può osservare in un resistore: quando attraversato da una corrente elettrica, esso si riscalda, dissipando parte dell'energia elettrica sotto forma di calore. Allo stesso modo, il funzionamento di una lampadina dimostra che il passaggio della corrente genera calore, rendendo incandescente il filamento e producendo energia luminosa.
...
L'elettrodo collegato al polo positivo del generatore è definito anodo, mentre quello collegato al polo negativo è chiamato catodo. Nel caso in cui si proceda a tale analisi utilizzando acqua distillata e un amperometro, si noterà che non si verifica alcun passaggio di corrente elettrica: l'acqua distillata si comporta dunque come un buon isolante. Questo fenomeno è attribuito alle molecole dell'acqua, le quali, in presenza di una differenza di potenziale applicata, non sono libere di muoversi all'interno del campo elettrico. Lo stesso comportamento isolante si riscontrerà in altri liquidi, quali l'alcol e l'olio.

Tuttavia, se nell'acqua viene disciolta una sostanza come il cloruro di sodio (NaCl) o l'acido cloridrico (HCl), la soluzione acquisisce proprietà conduttive. La sostanza che, una volta disciolta, conferisce al liquido capacità di condurre elettricità è definita elettrolita. I processi associati al passaggio della corrente attraverso una soluzione elettrolitica sono noti come fenomeni di elettrolisi.

Per tali liquidi, come per i solidi, si applica la legge di Ohm: l'intensità della corrente è proporzionale alla differenza di potenziale applicata agli elettrodi. Tuttavia, la resistività varia significativamente da liquido a liquido. Gli elettroliti, in generale, sono classificati come conduttori di seconda classe poiché presentano resistività notevolmente superiore rispetto ai conduttori metallici (conduttori di prima classe).

=== L'elettrolisi

L'elettrolisi può essere descritta come l'insieme dei fenomeni che si verificano in seguito al passaggio di corrente elettrica attraverso una soluzione contenente un elettrolita. Per comprendere la presenza di particelle cariche all'interno di tali soluzioni, è utile analizzare le forze chimiche che mantengono legate le particelle (atomi o ioni) nei composti. Nel caso di alcuni composti chimici come acidi, basi o sali, definiti elettroliti, la dissoluzione in acqua è accompagnata dalla dissociazione in ioni: atomi o gruppi di atomi dotati di carica elettrica. Gli ioni con carica positiva sono detti cationi, mentre quelli con carica negativa sono chiamati anioni.

La molecola d'acqua, composta da due atomi di idrogeno e uno di ossigeno, presenta caratteristiche analoghe a quelle di un dipolo elettrico; un dipolo elettrico è costituito da due cariche opposte di uguale intensità, separate da una distanza minima. Nella molecola d'acqua, l'atomo di ossigeno assume una frazione di carica negativa, mentre ciascun atomo di idrogeno ospita una frazione equivalente di carica positiva.
...
#boxfl(bottom)[== #wolf Il tubo a raggi catodici <senza_numero>
...
In condizioni normali, un gas è privo di portatori di carica e funge da buon isolante. Tuttavia, se sottoposto all'azione di un agente ionizzante, capace di generare coppie di ioni al suo interno, il gas può trasformarsi in un conduttore. Gli agenti ionizzanti includono raggi X, luce e altre radiazioni elettromagnetiche, che forniscono agli elettroni più esterni degli atomi la sufficiente energia per spezzare i legami che li legano ai nuclei atomici. Questo processo origina ioni positivi e negativi che, trasportando la corrente elettrica, rendono il gas conduttivo.

A differenza dei solidi metallici, i conduttori gassosi non obbediscono alla legge di Ohm: nel caso dei gas, la relazione tra intensità di corrente e differenza di potenziale non è lineare ma dipende dalla pressione del gas. Un fenomeno tipico in un sistema di gas sottoposto a una differenza di potenziale tra due elettrodi è la scintilla. Quando questa si verifica in un tubo contenente gas a pressione molto bassa, si espande lentamente fino a riempire tutto il tubo, generando una scarica luminosa. Tale principio è alla base del funzionamento dei tubi a luminescenza comunemente utilizzati per l'illuminazione.

Diminuendo ulteriormente la pressione all'interno di un tubo a scarica, il movimento delle particelle ionizzate diviene quasi nullo a causa della rarefazione del gas. Tuttavia, sulle pareti del tubo opposte al catodo appare una luminescenza, o fluorescenza, prodotta dal flusso di elettroni emessi dal catodo, sempre che la differenza di potenziale applicata risulti sufficientemente elevata.

Grazie alla loro origine, tali fasci di elettroni sono stati denominati raggi catodici. L'uso dei raggi catodici trova applicazione, ad esempio, nella generazione di immagini televisive.

Il tipo più semplice di tubo elettronico è rappresentato dal diodo, che può essere a vuoto oppure a gas. In una struttura di vetro sigillata, in cui è stato effettuato il vuoto, un filamento attraversato da corrente elettrica produce elettroni per effetto del riscaldamento. All'interno della stessa struttura si trova anche una placca metallica: se questa viene posta a un potenziale positivo rispetto al filamento (che assume dunque un potenziale negativo), gli elettroni emessi dal filamento vengono attratti e raccolti dalla placca, consentendo il passaggio della corrente. Viceversa, invertendo le polarità, gli elettroni sono respinti dalla placca e non circola alcuna corrente. Il diodo, pertanto, è caratterizzato dal fatto che permette il passaggio della corrente solo in una direzione, analogamente a quanto si verifica nei diodi a semiconduttore.

I diodi a gas si differenziano dai modelli a vuoto per la presenza, all'interno dell'ampolla, di un gas rarefatto invece del vuoto. Un fenomeno correlato alla conduzione della corrente attraverso un gas è rappresentato dai fulmini, esempi naturali di scariche elettriche nell'aria.
